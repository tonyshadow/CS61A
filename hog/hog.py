...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 2
    if num_rolls == 0:  #check if using free_bacon rule
        uncapped_score = free_bacon(opponent_score)
    else:
        uncapped_score = roll_dice(num_rolls, dice)  #regular rule
    if is_prime(uncapped_score):    #check if the score is prime
        uncapped_score = next_prime(uncapped_score)
    return min(uncapped_score, 25 - num_rolls)
    # END PROBLEM 2
